	...
	$(function(){
		...
		{	   
		  ...
		  //= parseFloat(dblTotalPrice).toFixed(parseInt(maxAmountDecimalPlaceLimit));
			...
			row.insertCell(3).innerHTML= "<input type=\"text\"  required = \"required\" required = \"required\" name=\"listReqDtl["+(rowCount)+"].dblQty\" id=\"dblQty."+(rowCount)+"\" value="+parseFloat(dblQty).toFixed(parseInt(maxAmountDecimalPlaceLimit))+" onblur=\"funUpdatePrice(this);\" onkeydown =\"funGetKeyCode(event, "+(rowCount)+");\" class=\"decimal-places inputText-Auto QtyCell\" >";	    
		    if(showReqVal=="N")
	    	{
			 ...
			 */
			 function funAddProductRow() 
				{			
				    ...
				    stock=parseFloat(stock).toFixed(parseInt(maxQuantityDecimalPlaceLimit));
...
<div id="somediv"></div>
	<div id="formHeading">
		<label id="lblFormHeadingName">Material Requisition</label>
	</div>
	<s:form name="matReq" method="POST" action="saveMaterialReq.html?saddr=${urlHits}">
		<br>
		...
		<table class="transTable">
			...
			<tr>
				...
				<td colspan="3"><input type="Button" value="Fill"
					onclick="return btnFill_onclick()" class="smallButton" /></td>
			</tr>
		</table>

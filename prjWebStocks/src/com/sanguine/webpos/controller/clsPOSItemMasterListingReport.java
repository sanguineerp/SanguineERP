...
import com.sanguine.webpos.bean.clsPOSItemMasterListingReportBean;
...
public class clsPOSItemMasterListingReport {
	...
	public ModelAndView funOpenForm(Map<String, Object> model, HttpServletRequest request) {
		...
		if ("2".equalsIgnoreCase(urlHits)) {
			return new ModelAndView("frmPOSItemMasterListingReport_1", "command", new clsWebPOSReportBean());
		} else if ("1".equalsIgnoreCase(urlHits)) {
			return new ModelAndView("frmPOSItemMasterListingReport", "command", new clsWebPOSReportBean());
		} else {
			return null;
		}
	}
	...
	private void funReport(@ModelAttribute("command") clsWebPOSReportBean objBean, HttpServletResponse resp, HttpServletRequest req) {
		...
		try {
			...
			JSONObject jObj = objGlobalFunctions.funPOSTMethodUrlJosnObjectData(clsPOSGlobalFunctionsController.POSWSURL + "/WebPOSReport/funItemMasterListingReport", jObjFillter);
			List<clsPOSItemMasterListingReportBean> list = new ArrayList<clsPOSItemMasterListingReportBean>();
			JSONArray jarr = (JSONArray) jObj.get("jArr");
			for (int i = 0; i < jarr.size(); i++) {
				JSONObject jObjtemp = (JSONObject) jarr.get(i);
				clsPOSItemMasterListingReportBean objItemMasterListingBean = new clsPOSItemMasterListingReportBean();
				objItemMasterListingBean.setStrItemCode(jObjtemp.get("strItemCode").toString());
				objItemMasterListingBean.setStrItemName(jObjtemp.get("strItemName").toString());
				objItemMasterListingBean.setStrSubGroupName(jObjtemp.get("strSubGroupName").toString());
				objItemMasterListingBean.setStrGroupName(jObjtemp.get("strGroupName").toString());
				objItemMasterListingBean.setStrTaxIndicator(jObjtemp.get("strTaxIndicator").toString());
				list.add(objItemMasterListingBean);
			}

			/*
			 * Comparator<clsPOSWaiterWiseItemWiseIncentivesReportBean>
			 * waiterCodeComparator = new
			 * Comparator<clsPOSWaiterWiseItemWiseIncentivesReportBean>() {
			 * 
			 * @Override public int
			 * compare(clsPOSWaiterWiseItemWiseIncentivesReportBean o1,
			 * clsPOSWaiterWiseItemWiseIncentivesReportBean o2) { return
			 * o1.getStrItemCode().compareToIgnoreCase(o2.getStrItemCode()); }
			 * };
			 * 
			 * Collections.sort(list, new
			 * clsWaiterWiseItemWiseIncentivesComparator(waiterCodeComparator)
			 * );
			 */
			...
			if (jprintlist.size() > 0) {
				...
				if (objBean.getStrDocType().equals("PDF")) {
					...
					resp.setHeader("Content-Disposition", "inline;filename=ItemMasterListing" + strFromdate + "_To_" + strToDate + "_" + userCode + ".pdf");
					...
					resp.setHeader("Content-Disposition", "inline;filename=ItemMasterListing" + strFromdate + "_To_" + strToDate + "_" + userCode + ".xls");
					exporter.exportReport();
					servletOutputStream.flush();
					servletOutputStream.close();
				}

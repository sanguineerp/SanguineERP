...
@Controller
public class clsPOSPromotionFlashReport {
	...
	Map<String, String> hmPromotinData;

	@RequestMapping(value = "/frmPOSPromationFlash", method = RequestMethod.GET)
	public ModelAndView funOpenForm(Map<String, Object> model, HttpServletRequest request) {
		...
		PromotionList.add("All");
		hmPromotinData = new HashMap<String, String>();
		JSONArray jArryReasonList = objPOSGlobalFunctionsController.funGetAllPromotionMaster(strClientCode);
		for (int i = 0; i < jArryReasonList.size(); i++) {
			JSONObject josnObjRet = (JSONObject) jArryReasonList.get(i);
			PromotionList.add(josnObjRet.get("PromoName").toString());
			hmPromotinData.put(josnObjRet.get("PromoName").toString(), josnObjRet.get("PromoCode").toString());
		}
		model.put("PromotionList", PromotionList);
		...
		if ("2".equalsIgnoreCase(urlHits)) {
			return new ModelAndView("frmPOSPromationFlash_1", "command", new clsWebPOSReportBean());
		} else if ("1".equalsIgnoreCase(urlHits)) {
			return new ModelAndView("frmPOSPromationFlash", "command", new clsWebPOSReportBean());
		} else {
			return null;
		}
	...
	@RequestMapping(value = "/rptPromotionFlashReport", method = RequestMethod.POST)
	private void funReport(@ModelAttribute("command") clsWebPOSReportBean objBean, HttpServletResponse resp, HttpServletRequest req) {
		...
		try {
			...
			String reportName = servletContext.getRealPath("/WEB-INF/reports/webpos/rptPromotionFlash.jrxml");
			...
			String posCode = "All";
			String PromoCode = "All";
			String PromoName = objBean.getStrPromotionCode();

			if (!strPOSName.equalsIgnoreCase("All")) {
				posCode = hmPOSData.get(strPOSName);
			}

			if (!PromoName.equalsIgnoreCase("All")) {
				PromoCode = hmPromotinData.get(PromoName);
			}
			...
			jObjFillter.put("PromoCode", PromoCode);

			JSONObject jObj = objGlobalFunctions.funPOSTMethodUrlJosnObjectData("http://localhost:8080/prjSanguineWebService/WebPOSReport/funPromotionFlash", jObjFillter);
			List<clsBillItemDtlBean> listBillItemDtlBean = new ArrayList<clsBillItemDtlBean>();
			...
			for (int i = 0; i < jarr.size(); i++) {
				...
				objBillItemDtlBean.setStrBillNo(jObjtemp.get("strBillNo").toString());
				objBillItemDtlBean.setDteBillDate(jObjtemp.get("dteBillDate").toString());
				objBillItemDtlBean.setStrItemCode(jObjtemp.get("strItemCode").toString());
				objBillItemDtlBean.setStrItemName(jObjtemp.get("strItemName").toString());
				objBillItemDtlBean.setDblQuantity(Double.parseDouble(jObjtemp.get("dblQuantity").toString()));
				;
				objBillItemDtlBean.setDblAmount(Double.parseDouble(jObjtemp.get("dblAmount").toString()));
				;

				listBillItemDtlBean.add(objBillItemDtlBean);
			}
			...
			JRBeanCollectionDataSource beanCollectionDataSource = new JRBeanCollectionDataSource(listBillItemDtlBean);
			...
			if (jprintlist.size() > 0) {
				...
				if (objBean.getStrDocType().equals("PDF")) {
					...
					resp.setHeader("Content-Disposition", "inline;filename=PromotionReport_" + strFromdate + "_To_" + strToDate + "_" + userCode + ".pdf");
					...
					resp.setHeader("Content-Disposition", "inline;filename=PromotionReport_" + strFromdate + "_To_" + strToDate + "_" + userCode + ".xls");
					exporter.exportReport();
					servletOutputStream.flush();
					servletOutputStream.close();
				}

...
@Controller
public class clsPOSCounterWiseSalesReport {
	...
	public ModelAndView funOpenForm(Map<String, Object> model, HttpServletRequest request) {
		...
		if ("2".equalsIgnoreCase(urlHits)) {
			return new ModelAndView("frmPOSCounterWiseSalesReport_1", "command", new clsWebPOSReportBean());
		} else if ("1".equalsIgnoreCase(urlHits)) {
			return new ModelAndView("frmPOSCounterWiseSalesReport", "command", new clsWebPOSReportBean());
		} else {
			return null;
		}
	...
	@RequestMapping(value = "/rptCounterWiseSalesReportReport", method = RequestMethod.POST)
	private void funReport(@ModelAttribute("command") clsWebPOSReportBean objBean, HttpServletResponse resp, HttpServletRequest req) {
		...
		try {
			...
			List<clsCounterDtlBean> listOfCounter = new ArrayList<>();
			...
			JSONObject jObj = objGlobalFunctions.funPOSTMethodUrlJosnObjectData(clsPOSGlobalFunctionsController.POSWSURL + "/WebPOSReport/funCounterWiseSalesReport", jObjFillter);
			Gson gson = new Gson();
			Type listType = new TypeToken<List<clsCounterDtlBean>>() {
			}.getType();
			JRBeanCollectionDataSource beanCollectionDataSource;
			if (objBean.getStrType().equals("Menu Wise")) {
				listOfCounter = gson.fromJson(jObj.get("ListCounterWiseMenuHead").toString(), listType);
				beanCollectionDataSource = new JRBeanCollectionDataSource(listOfCounter);

			} else if (objBean.getStrType().equals("Group Wise")) {
				listOfCounter = gson.fromJson(jObj.get("ListCounterGroupWise").toString(), listType);
				beanCollectionDataSource = new JRBeanCollectionDataSource(listOfCounter);
			} else {
				listOfCounter = gson.fromJson(jObj.get("ListCounterSubGroupWise").toString(), listType);
				beanCollectionDataSource = new JRBeanCollectionDataSource(listOfCounter);
			}
			...
			if (jprintlist.size() > 0) {
				...
				if (objBean.getStrDocType().equals("PDF")) {
					...
					resp.setHeader("Content-Disposition", "inline;filename=CounterWiseReport_" + reportType + "_" + strFromdate + "_To_" + strToDate + "_" + userCode + ".pdf");
					...
					resp.setHeader("Content-Disposition", "inline;filename=CounterWiseReport_" + reportType + "_" + strFromdate + "_To_" + strToDate + "_" + userCode + ".xls");
					exporter.exportReport();
					servletOutputStream.flush();
					servletOutputStream.close();
				}

...
public class clsPOSSettlementWiseReportController {
	...
	public ModelAndView funOpenForm(Map<String, Object> model, HttpServletRequest request) {
		...
		if ("2".equalsIgnoreCase(urlHits)) {
			return new ModelAndView("frmPOSSettlementWiseReport_1", "command", new clsWebPOSReportBean());
		} else if ("1".equalsIgnoreCase(urlHits)) {
			return new ModelAndView("frmPOSSettlementWiseReport", "command", new clsWebPOSReportBean());
		} else {
			return null;
		}
	...
	private void funReport(@ModelAttribute("command") clsWebPOSReportBean objBean, HttpServletResponse resp, HttpServletRequest req) {
		...
		try {
			...
			JSONObject jObj = objGlobalFunctions.funPOSTMethodUrlJosnObjectData("http://localhost:8080/prjSanguineWebService/WebPOSReport/funSettlementWiseSales", jObjFillter);
			List<clsPOSSettlementWiseSalesReportBean> list = new ArrayList<clsPOSSettlementWiseSalesReportBean>();
			JSONArray jarr = (JSONArray) jObj.get("jArr");
			for (int i = 0; i < jarr.size(); i++) {
				JSONObject jObjtemp = (JSONObject) jarr.get(i);

				clsPOSSettlementWiseSalesReportBean objPOSSettlementWiseSalesReportBean = new clsPOSSettlementWiseSalesReportBean();
				objPOSSettlementWiseSalesReportBean.setStrPosName(jObjtemp.get("strposname").toString());
				objPOSSettlementWiseSalesReportBean.setStrSettelmentMode(jObjtemp.get("strSettelmentDesc").toString());
				objPOSSettlementWiseSalesReportBean.setDblSettlementAmt(Double.parseDouble(jObjtemp.get("dblSettlementAmt").toString()));
				;

				list.add(objPOSSettlementWiseSalesReportBean);
			}
			...
			if (jprintlist.size() > 0) {
				...
				if (objBean.getStrDocType().equals("PDF")) {
					...
					resp.setHeader("Content-Disposition", "inline;filename=SettlementWiseReport_" + strFromdate + "_To_" + strToDate + "_" + userCode + ".pdf");
					...
					resp.setHeader("Content-Disposition", "inline;filename=SettlementWiseReport_" + strFromdate + "_To_" + strToDate + "_" + userCode + ".xls");
					exporter.exportReport();
					servletOutputStream.flush();
					servletOutputStream.close();
				}

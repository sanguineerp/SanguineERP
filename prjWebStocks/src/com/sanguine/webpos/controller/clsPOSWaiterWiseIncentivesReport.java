...
@Controller
public class clsPOSWaiterWiseIncentivesReport {
	...
	public ModelAndView funOpenForm(Map<String, Object> model, HttpServletRequest request) {
		...
		if ("2".equalsIgnoreCase(urlHits)) {
			return new ModelAndView("frmPOSWaiterWiseIncentiveReport_1", "command", new clsWebPOSReportBean());
		} else if ("1".equalsIgnoreCase(urlHits)) {
			return new ModelAndView("frmPOSWaiterWiseIncentiveReport", "command", new clsWebPOSReportBean());
		} else {
			return null;
		}
	...
	private void funReport(@ModelAttribute("command") clsWebPOSReportBean objBean, HttpServletResponse resp, HttpServletRequest req) {
		...
		try {
			if (objBean.getStrType().equals("Summary")) {
				reportName = servletContext.getRealPath("/WEB-INF/reports/webpos/rptWaiterWiseIncentivesSummaryReport.jrxml");
				imagePath = servletContext.getRealPath("/resources/images/company_Logo.png");//

			} else {
				reportName = servletContext.getRealPath("/WEB-INF/reports/webpos/rptWaiterWiseIncentivesDetailsReport.jrxml");
				imagePath = servletContext.getRealPath("/resources/images/company_Logo.png");//
			...
			JSONObject jObj = objGlobalFunctions.funPOSTMethodUrlJosnObjectData(clsPOSGlobalFunctionsController.POSWSURL + "/WebPOSReport/funWaiterWiseIncentiveReport", jObjFillter);
			Gson gson = new Gson();
			Type listType = new TypeToken<List<clsBillDtl>>() {
			}.getType();
			JRBeanCollectionDataSource beanCollectionDataSource;
			if (objBean.getStrType().equals("Summary")) {
				listOfWaiterWiseItemSales = gson.fromJson(jObj.get("ListWaiterWiseIncentiveSummary").toString(), listType);
				beanCollectionDataSource = new JRBeanCollectionDataSource(listOfWaiterWiseItemSales);
			} else {
				listOfWaiterWiseItemSales = gson.fromJson(jObj.get("ListWaiterWiseIncentiveDetail").toString(), listType);
				beanCollectionDataSource = new JRBeanCollectionDataSource(listOfWaiterWiseItemSales);
			}
			...
			if (jprintlist.size() > 0) {
				...
				if (objBean.getStrDocType().equals("PDF")) {
					...
					resp.setHeader("Content-Disposition", "inline;filename=WaiterWiseIncentiveReport_" + reportType + "_" + strFromdate + "_To_" + strToDate + "_" + userCode + ".pdf");
					...
					resp.setHeader("Content-Disposition", "inline;filename=WaiterWiseIncentiveReport_" + reportType + "_" + strFromdate + "_To_" + strToDate + "_" + userCode + ".xls");
					exporter.exportReport();
					servletOutputStream.flush();
					servletOutputStream.close();
				}

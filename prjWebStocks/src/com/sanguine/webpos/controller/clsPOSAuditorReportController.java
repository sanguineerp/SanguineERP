...
public class clsPOSAuditorReportController {
	...
	public ModelAndView funOpenForm(Map<String, Object> model, HttpServletRequest request) {
		...
		if ("2".equalsIgnoreCase(urlHits)) {
			return new ModelAndView("frmPOSAuditorReport_1", "command", new clsWebPOSReportBean());
		} else if ("1".equalsIgnoreCase(urlHits)) {
			return new ModelAndView("frmPOSAuditorReport", "command", new clsWebPOSReportBean());
		} else {
			return null;
		}
	...
	private void funReport(@ModelAttribute("command") clsWebPOSReportBean objBean, HttpServletResponse resp, HttpServletRequest req) {
		...
		List<clsPOSAuditorReportBean> listOperatorDtl = new ArrayList<>();
		List<clsPOSAuditorReportBean> listSettleDetail = new ArrayList<>();
		List<clsPOSAuditorReportBean> listVoidBillDetail = new ArrayList<>();
		...
		try {
			...
			JSONObject jObj = objGlobalFunctions.funPOSTMethodUrlJosnObjectData("http://localhost:8080/prjSanguineWebService/WebPOSReport/funAuditorReport", jObjFillter);
			List<clsPOSAuditorReportBean> list = new ArrayList<clsPOSAuditorReportBean>();
			// JSONArray jarr = (JSONArray) jObj.get("jArr");

			MinBillNo = jObj.get("minBillNo").toString();
			MaxBillNo = jObj.get("maxBillNo").toString();
			TotalDiscount = jObj.get("totalDiscount").toString();

			/*
			 * listOperatorDtl=(List<clsPOSAuditorReportBean>)
			 * jObj.get("listOfOperatorDtl");
			 * listSettleDetail=(List<clsPOSAuditorReportBean>)
			 * jObj.get("listOfBillSettleDtl");
			 * listVoidBillDetail=(List<clsPOSAuditorReportBean>)
			 * jObj.get("listOfVoidBillDtl");
			 */

			Gson gson = new Gson();
			Type listType = new TypeToken<List<clsPOSAuditorReportBean>>() {
			}.getType();

			listOperatorDtl = gson.fromJson(jObj.get("listOfOperatorDtl").toString(), listType);
			listSettleDetail = gson.fromJson(jObj.get("listOfBillSettleDtl").toString(), listType);
			listVoidBillDetail = gson.fromJson(jObj.get("listOfVoidBillDtl").toString(), listType);

			// gson.fromJson(, );

			/*
			 * System.out.println(listOperatorDtl.get(0).getStrUserCode());
			 * System.out.println(listSettleDetail.get(0).getStrUserCode());
			 * System.out.println(listVoidBillDetail.get(0).getStrUserCode());
			 */
			...
			// listOfBillSettleDtl listOfVoidBillDtl listOfOperatorDtl

			List<List<clsPOSAuditorReportBean>> listData = new ArrayList<>();
			...
			if (jprintlist.size() > 0) {
				...
				if (objBean.getStrDocType().equals("PDF")) {
					...
					resp.setHeader("Content-Disposition", "inline;filename=AuditorReport_" + strFromdate + "_To_" + strToDate + "_" + userCode + ".pdf");
					...
					resp.setHeader("Content-Disposition", "inline;filename=AuditorReport_" + strFromdate + "_To_" + strToDate + "_" + userCode + ".xls");
					exporter.exportReport();
					servletOutputStream.flush();
					servletOutputStream.close();
				}

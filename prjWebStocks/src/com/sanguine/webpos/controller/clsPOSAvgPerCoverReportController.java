...
@Controller
public class clsPOSAvgPerCoverReportController {
	...
	public ModelAndView funOpenForm(Map<String, Object> model, HttpServletRequest request) {
		...
		if ("2".equalsIgnoreCase(urlHits)) {
			return new ModelAndView("frmPOSAvgPerCoverReport_1", "command", new clsWebPOSReportBean());
		} else if ("1".equalsIgnoreCase(urlHits)) {
			return new ModelAndView("frmPOSAvgPerCoverReport", "command", new clsWebPOSReportBean());
		} else {
			return null;
		}
	}
	...
	private void funReport(@ModelAttribute("command") clsWebPOSReportBean objBean, HttpServletResponse resp, HttpServletRequest req) {
		...
		try {
			String reportName = servletContext.getRealPath("/WEB-INF/reports/webpos/rptAPC.jrxml");
			...
			String strPowWise = objBean.getStrPosWise();
			...
			jObjFillter.put("strPowWise", strPowWise);
			jObjFillter.put("strDateWise", strDateWise);
			jObjFillter.put("strShiftNo", "1");

			JSONObject jObj = objGlobalFunctions.funPOSTMethodUrlJosnObjectData(clsPOSGlobalFunctionsController.POSWSURL + "/WebPOSReport/funAPCReport", jObjFillter);
			...
			if (jprintlist.size() > 0) {
				...
				if (objBean.getStrDocType().equals("PDF")) {
					...
					resp.setHeader("Content-Disposition", "inline;filename=POSAPCReport" + strFromdate + "_To_" + strToDate + "_" + userCode + ".pdf");
					...
					resp.setHeader("Content-Disposition", "inline;filename=POSAPCReport" + strFromdate + "_To_" + strToDate + "_" + userCode + ".xls");
					exporter.exportReport();
					servletOutputStream.flush();
					servletOutputStream.close();
				}

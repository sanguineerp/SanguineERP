...
public class clsPOSTaxBreakupSummaryReport {
	...
	public ModelAndView funOpenForm(Map<String, Object> model, HttpServletRequest request) {
		...
		if ("2".equalsIgnoreCase(urlHits)) {
			return new ModelAndView("funPOSTaxBreakupSummaryReport_1", "command", new clsWebPOSReportBean());
		} else if ("1".equalsIgnoreCase(urlHits)) {
			return new ModelAndView("funPOSTaxBreakupSummaryReport", "command", new clsWebPOSReportBean());
		} else {
			return null;
		}
	...
	private void funReport(@ModelAttribute("command") clsWebPOSReportBean objBean, HttpServletResponse resp, HttpServletRequest req) {
		...
		try {
			...
			JSONObject jObj = objGlobalFunctions.funPOSTMethodUrlJosnObjectData(clsPOSGlobalFunctionsController.POSWSURL + "/WebPOSReport/funTaxBreakupSummaryReport", jObjFillter);

			Gson gson = new Gson();
			Type listType = new TypeToken<List<clsTaxCalculationDtls>>() {
			}.getType();
			listOfTaxDtl = gson.fromJson(jObj.get("listOfTaxDtl").toString(), listType);
			listOfMenuHeadBreakupDtl = gson.fromJson(jObj.get("listOfMenuHeadBreakupDtl").toString(), listType);
			...
			List<List<clsTaxCalculationDtls>> listData = new ArrayList<>();
			listData.add(listOfTaxDtl);
			...
			if (jprintlist.size() > 0) {
				...
				if (objBean.getStrDocType().equals("PDF")) {
					...
					resp.setHeader("Content-Disposition", "inline;filename=TaxBreakSummaryReport_" + strFromdate + "_To_" + strToDate + "_" + userCode + ".pdf");
					...
					resp.setHeader("Content-Disposition", "inline;filename=TaxBreakSummaryReport_" + strFromdate + "_To_" + strToDate + "_" + userCode + ".xls");
					exporter.exportReport();
					servletOutputStream.flush();
					servletOutputStream.close();
				}

...
public class clsPOSGuestCreditReportController {
	...
	public ModelAndView funOpenForm(Map<String, Object> model, HttpServletRequest request) {
		...
		if ("2".equalsIgnoreCase(urlHits)) {
			return new ModelAndView("frmPOSGuestCreditReport_1", "command", new clsWebPOSReportBean());
		} else if ("1".equalsIgnoreCase(urlHits)) {
			return new ModelAndView("frmPOSGuestCreditReport", "command", new clsWebPOSReportBean());
		} else {
			return null;
		}
	...
	private void funReport(@ModelAttribute("command") clsWebPOSReportBean objBean, HttpServletResponse resp, HttpServletRequest req) {
		...
		try {
			...
			JSONObject jObj = objGlobalFunctions.funPOSTMethodUrlJosnObjectData("http://localhost:8080/prjSanguineWebService/WebPOSReport/funGuestCreditReport", jObjFillter);
			List<clsBillDtl> listOfGuestCreditData = new ArrayList<clsBillDtl>();

			Gson gson = new Gson();
			Type listType = new TypeToken<List<clsBillDtl>>() {
			}.getType();
			listOfGuestCreditData = gson.fromJson(jObj.get("listOfGuestCreditData").toString(), listType);
			...
			if (jprintlist.size() > 0) {
				...
				if (objBean.getStrDocType().equals("PDF")) {
					...
					resp.setHeader("Content-Disposition", "inline;filename=GuestCreditReport_" + strFromdate + "_To_" + strToDate + "_" + userCode + ".pdf");
					...
					resp.setHeader("Content-Disposition", "inline;filename=GuestCreditReport_" + strFromdate + "_To_" + strToDate + "_" + userCode + ".xls");
					exporter.exportReport();
					servletOutputStream.flush();
					servletOutputStream.close();
				}

...
@Controller
public class clsPOSUnusedCardBalanceReportController {
	...
	@RequestMapping(value = "/frmPOSUnusedCardBalanceReport", method = RequestMethod.GET)
	public ModelAndView funOpenForm(Map<String, Object> model, HttpServletRequest request) {
		...
		if ("2".equalsIgnoreCase(urlHits)) {
			return new ModelAndView("frmPOSUnusedCardBalanceReport_1", "command", new clsWebPOSReportBean());
		} else if ("1".equalsIgnoreCase(urlHits)) {
			return new ModelAndView("frmPOSUnusedCardBalanceReport", "command", new clsWebPOSReportBean());
		} else {
			return null;
		}
	}
	...
	@RequestMapping(value = "/rptPOSUnusedCardBalanceReport", method = RequestMethod.POST)
	private void funReport(@ModelAttribute("command") clsWebPOSReportBean objBean, HttpServletResponse resp, HttpServletRequest req) {
		...
		try {
			String reportName = servletContext.getRealPath("/WEB-INF/reports/webpos/rptUnusedCardBalance.jrxml");
			...
			JSONObject jObj = objGlobalFunctions.funPOSTMethodUrlJosnObjectData("http://localhost:8080/prjSanguineWebService/WebPOSReport/funUnusedCardBalanceReport", jObjFillter);
			List<clsGenericBean> listOfUnUsedBalance = new ArrayList<clsGenericBean>();

			Gson gson = new Gson();
			Type listType = new TypeToken<List<clsGenericBean>>() {
			}.getType();
			listOfUnUsedBalance = gson.fromJson(jObj.get("listOfUnUsedBalance").toString(), listType);
			...
			JRBeanCollectionDataSource beanCollectionDataSource = new JRBeanCollectionDataSource(listOfUnUsedBalance);
			...
			if (jprintlist.size() > 0) {
				...
				if (objBean.getStrDocType().equals("PDF")) {
					...
					resp.setHeader("Content-Disposition", "inline;filename=UnusedCardBalanceReport_" + strFromdate + "_To_" + strToDate + "_" + userCode + ".pdf");
					...
					resp.setHeader("Content-Disposition", "inline;filename=UnusedCardBalanceReport_" + strFromdate + "_To_" + strToDate + "_" + userCode + ".xls");
					exporter.exportReport();
					servletOutputStream.flush();
					servletOutputStream.close();
				}

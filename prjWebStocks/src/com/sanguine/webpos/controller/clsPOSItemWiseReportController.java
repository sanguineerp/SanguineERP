...
public class clsPOSItemWiseReportController {
	...
	private void funReport(@ModelAttribute("command") clsWebPOSReportBean objBean, HttpServletResponse resp, HttpServletRequest req) {
		...
		try {
			...
			JSONObject jObj = objGlobalFunctions.funPOSTMethodUrlJosnObjectData("http://localhost:8080/prjSanguineWebService/WebPOSReport/funItemWiseSalesReport", jObjFillter);
			List<clsPOSItemWiseSalesReportBean> list = new ArrayList<clsPOSItemWiseSalesReportBean>();
			JSONArray jarr = (JSONArray) jObj.get("jArr");
			for (int i = 0; i < jarr.size(); i++) {
				JSONObject jObjtemp = (JSONObject) jarr.get(i);

				clsPOSItemWiseSalesReportBean objPOSItemWiseSalesReportBean = new clsPOSItemWiseSalesReportBean();
				objPOSItemWiseSalesReportBean.setStrItemCode(jObjtemp.get("strItemCode").toString());
				objPOSItemWiseSalesReportBean.setStrItemName(jObjtemp.get("strItemName").toString());
				objPOSItemWiseSalesReportBean.setDblQuantity(Double.parseDouble(jObjtemp.get("dblQuantity").toString()));
				;
				objPOSItemWiseSalesReportBean.setDblAmount(Double.parseDouble(jObjtemp.get("dblAmount").toString()));
				objPOSItemWiseSalesReportBean.setDblSubTotal(Double.parseDouble(jObjtemp.get("dblSubTotal").toString()));
				objPOSItemWiseSalesReportBean.setDblDiscountAmt(Double.parseDouble(jObjtemp.get("dblDiscountAmt").toString()));

				list.add(objPOSItemWiseSalesReportBean);
			}
			...
			if (jprintlist.size() > 0) {
				...
				if (objBean.getStrDocType().equals("PDF")) {
					...
					resp.setHeader("Content-Disposition", "inline;filename=ItemWiseSalesReport_" + strFromdate + "_To_" + strToDate + "_" + userCode + ".xls");
					exporter.exportReport();
					servletOutputStream.flush();
					servletOutputStream.close();
				}

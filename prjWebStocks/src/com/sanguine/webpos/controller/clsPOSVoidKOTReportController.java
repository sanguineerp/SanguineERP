...
public class clsPOSVoidKOTReportController {
	...
	public ModelAndView funOpenForm(Map<String, Object> model, HttpServletRequest request) {
		...
		if ("2".equalsIgnoreCase(urlHits)) {
			return new ModelAndView("frmPOSVoidKOTReport_1", "command", new clsWebPOSReportBean());
		} else if ("1".equalsIgnoreCase(urlHits)) {
			return new ModelAndView("frmPOSVoidKOTReport", "command", new clsWebPOSReportBean());
		} else {
			return null;
		}
	...
	private void funReport(@ModelAttribute("command") clsWebPOSReportBean objBean, HttpServletResponse resp, HttpServletRequest req) {
		...
		try {
			...
			JSONObject jObj = objGlobalFunctions.funPOSTMethodUrlJosnObjectData(clsPOSGlobalFunctionsController.POSWSURL + "/WebPOSReport/funVoidKOTReport", jObjFillter);
			...
			for (int i = 0; i < jarr.size(); i++) {
				...
				objReportBean.setStrTableNo(jObjtemp.get("strTableNo").toString());
				objReportBean.setDblPaidAmt(Double.parseDouble(jObjtemp.get("dblPaidAmt").toString()));
				objReportBean.setStrRemarks(jObjtemp.get("strRemarks").toString());
				objReportBean.setDblQuantity(Double.parseDouble(jObjtemp.get("intQuantity").toString()));
				objReportBean.setDblAmount(Double.parseDouble(jObjtemp.get("dblAmount").toString()));
				objReportBean.setStrKOTNo(jObjtemp.get("strKOTNo").toString());
				objReportBean.setStrClientCode(jObjtemp.get("strClientCode").toString());
				objReportBean.setStrPOSCode(jObjtemp.get("strPosCode").toString());
				objReportBean.setStrUserCreated(jObjtemp.get("strUserCreated").toString());
				objReportBean.setStrReasonMaster(jObjtemp.get("strReasonName").toString());

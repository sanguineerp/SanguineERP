...
@Controller
public class clsPOSDeliveryboyIncentive {
	@Autowired
	...
	Map<String, String> hmDeliveryBoy;

	@RequestMapping(value = "/frmPOSDeliveryboyIncentive", method = RequestMethod.GET)
	public ModelAndView funOpenForm(Map<String, Object> model, HttpServletRequest request) {
		...
		List DeliveryBoyList = new ArrayList();
		DeliveryBoyList.add("All");
		hmDeliveryBoy = new HashMap<String, String>();
		JSONArray jArryReasonList = objPOSGlobalFunctionsController.funGetAllDeliveryBoy(strClientCode);
		for (int i = 0; i < jArryReasonList.size(); i++) {
			JSONObject josnObjRet = (JSONObject) jArryReasonList.get(i);
			DeliveryBoyList.add(josnObjRet.get("DBName").toString());
			hmDeliveryBoy.put(josnObjRet.get("DBName").toString(), josnObjRet.get("DBCode").toString());
		}
		model.put("DeliveryBoyList", DeliveryBoyList);
		...
		if ("2".equalsIgnoreCase(urlHits)) {
			return new ModelAndView("frmPOSDeliveryboyIncentive_1", "command", new clsWebPOSReportBean());
		} else if ("1".equalsIgnoreCase(urlHits)) {
			return new ModelAndView("frmPOSDeliveryboyIncentive", "command", new clsWebPOSReportBean());
		} else {
			return null;
		}
	...
	@RequestMapping(value = "/rptDeliveryboyIncentive", method = RequestMethod.POST)
	private void funReport(@ModelAttribute("command") clsWebPOSReportBean objBean, HttpServletResponse resp, HttpServletRequest req) {
		...
		String imagePath = "";
		try {
			if (objBean.getStrType().equals("Summary")) {
				reportName = servletContext.getRealPath("/WEB-INF/reports/webpos/rptDeliveryBoyIncentiveSummaryReport.jrxml");
				imagePath = servletContext.getRealPath("/resources/images/company_Logo.png");//

			} else {
				reportName = servletContext.getRealPath("/WEB-INF/reports/webpos/rptDeliveryBoyIncentiveDetailReport.jrxml");
				imagePath = servletContext.getRealPath("/resources/images/company_Logo.png");//
			...
			String DBCode = "All";
			String DBName = objBean.getStrDeliveryBoyName();

			if (!strPOSName.equalsIgnoreCase("All")) {
				posCode = hmPOSData.get(strPOSName);
			}
			if (!DBName.equalsIgnoreCase("All")) {
				DBCode = hmDeliveryBoy.get(DBName);
			}
			...
			JSONObject jObj = objGlobalFunctions.funPOSTMethodUrlJosnObjectData(clsPOSGlobalFunctionsController.POSWSURL + "/WebPOSReport/funDeliveryBoyWiseIncentiveReport", jObjFillter);
			Gson gson = new Gson();
			Type listType = new TypeToken<List<clsBillDtl>>() {
			}.getType();
			JRBeanCollectionDataSource beanCollectionDataSource;
			if (objBean.getStrType().equals("Summary")) {
				listOfDelBoyIncentives = gson.fromJson(jObj.get("ListDBWiseIncentiveSummary").toString(), listType);
				beanCollectionDataSource = new JRBeanCollectionDataSource(listOfDelBoyIncentives);
			} else {
				listOfDelBoyIncentives = gson.fromJson(jObj.get("ListDBWiseIncentiveDetails").toString(), listType);
				beanCollectionDataSource = new JRBeanCollectionDataSource(listOfDelBoyIncentives);
			}
			...
			if (jprintlist.size() > 0) {
				...
				if (objBean.getStrDocType().equals("PDF")) {
					...
					resp.setHeader("Content-Disposition", "inline;filename=DeliveryBoyWiseIncentiveReport_" + reportType + "_" + strFromdate + "_To_" + strToDate + "_" + userCode + ".pdf");
					...
					resp.setHeader("Content-Disposition", "inline;filename=DeliveryBoyWiseIncentiveReport_" + reportType + "_" + strFromdate + "_To_" + strToDate + "_" + userCode + ".xls");
					exporter.exportReport();
					servletOutputStream.flush();
					servletOutputStream.close();
				}

package com.sanguine.bean;

public class clsPurchaseRegisterReportBean {

	private String strPName;

	private String strProdName;

	private double dblQty;

	private String strUOM;

	private double dblUnitPrice;

	private double dblAmount;

	private String dtGRNDate;
	
	private String strFromLocation;
	
	private String strBillNo;

	public String getStrPName() {
		return strPName;
	}

	public void setStrPName(String strPName) {
		this.strPName = strPName;
	}

	public String getStrProdName() {
		return strProdName;
	}

	public void setStrProdName(String strProdName) {
		this.strProdName = strProdName;
	}

	public double getDblQty() {
		return dblQty;
	}

	public void setDblQty(double dblQty) {
		this.dblQty = dblQty;
	}

	public String getStrUOM() {
		return strUOM;
	}

	public void setStrUOM(String strUOM) {
		this.strUOM = strUOM;
	}

	public double getDblUnitPrice() {
		return dblUnitPrice;
	}

	public void setDblUnitPrice(double dblUnitPrice) {
		this.dblUnitPrice = dblUnitPrice;
	}

	public double getDblAmount() {
		return dblAmount;
	}

	public void setDblAmount(double dblAmount) {
		this.dblAmount = dblAmount;
	}

	public String getDtGRNDate() {
		return dtGRNDate;
	}

	public void setDtGRNDate(String dtGRNDate) {
		this.dtGRNDate = dtGRNDate;
	}

	public String getStrFromLocation() {
		return strFromLocation;
	}

	public void setStrFromLocation(String strFromLocation) {
		this.strFromLocation = strFromLocation;
	}

	public String getStrBillNo() {
		return strBillNo;
	}

	public void setStrBillNo(String strBillNo) {
		this.strBillNo = strBillNo;
	}

}
